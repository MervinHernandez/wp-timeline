...
<li <?php post_class($class);?> <?php echo 'data-id="filter-'.$ID.'_'.get_the_ID().'"';?>>
    <div class="<?php echo esc_attr($animations);?>">
        ...
        <div class="wpex-timeline-label">
            <!-- SPECIAL BOX -->
            <div style="background-color: white;width:100%;padding: 10px 12px 10px 12px;margin-bottom: 2em;" id="merv-special-box">
            <?php
            echo 'WPEX_Order: '.get_post_meta( get_the_ID(), 'wpex_order', true).'<br>';
            echo 'Post ID: '.get_the_ID().'<br>';
            echo 'Sublabel: '.get_post_meta( get_the_ID(), 'wpex_sublabel', true).'<br>';
            echo  'Locale: ' .get_post_meta( get_the_ID(), 'wpex_locale', true).'<br>';
            echo 'WPEX_Date: '.get_post_meta( get_the_ID(), 'wpex_date', true).'<br>';            
            ?>                
            </div>
            <!-- special box -->
            <?php 
            ...
            <div class="timeline-details">                
                ...
                <!--Waldo by Mervin-->
                <h3>
                <?php echo get_post_meta( get_the_ID(), 'wpex_sublabel', true).'<br/>'.
                           get_post_meta( get_the_ID(), 'wpex_locale', true)?><br/>
                </h3>
                <!--Waldo-->
                <?php 
				...
				if($full_content=='1' && $show_media=='1'){
						...
						$content = preg_replace('%<object.+?</object>%is', '', $content,1);
						echo 'Filtered Content: '.apply_filters('the_content',$content);
					}elseif($full_content=='1'){
						echo 'Filtered Content: '.apply_filters('the_content', get_the_content());
					}else{
