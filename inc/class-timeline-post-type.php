...
class WPEX_TL_Posttype {
	...
	function add_meta_date_order($post_id){
		if(isset($_POST['wpex_pkdate'])){
			...
			if(!empty($order_mtk)){
				update_post_meta( $post_id, 'wptl_orderdate', $order_mtk[2].$order_mtk[1].$order_mtk[0] );
			}
		}
	}
	...
	function register_metadata(array $meta_boxes){
		...
		$speaker_settings = array(	
				array( 'id' => 'wpex_order', 'name' => esc_html__('Custom Order in Timeline', 'wp-timeline'), 'default'=> '0', 'type' => 'text','desc' => esc_html__('Enter number (if you use custom date then you can use this feature to order timeline)', 'wp-timeline') , 'repeatable' => false, 'multiple' => false ),
