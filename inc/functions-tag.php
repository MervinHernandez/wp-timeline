...
if( !function_exists('wpex_tmbigdate')){
	function wpex_tmbigdate($show_thumb=false,$link_lb=false){
		...
		//Waldo
        $month = DateTime::createFromFormat('m/d/Y', $wpex_date)->format('M');
        $year = DateTime::createFromFormat('m/d/Y', $wpex_date)->format('Y');
        $html ='
		<div class="wpex-content-left">
            <div class="wpex-leftdate">
                <span class="tlday">'.$month.'</span>
                <span class="tlday2">'.$year.'</span>
            </div>';
        //Waldo
